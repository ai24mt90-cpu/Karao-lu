...
export async function generateMetadata({ params }: { params: { kategori: string } }): Promise<Metadata> {
    ...
    const title = titleMap[params.kategori] || "Projeler";
...
async function getProjects(kategori: string) {
    let query = supabase
        .from("projects")
        .select("*")
    ...
    if (kategori === "tamamlanan-kamu-projeleri") {
        query = query.eq("status", "Tamamlandı");
    } else if (kategori === "devam-eden-altyapi-isleri") {
...
export default async function ProjectsPage({ params }: { params: { kategori: string } }) {
    const allowedCategories = ["tamamlanan-kamu-projeleri", "devam-eden-altyapi-isleri", "tum-projeler"];

    if (!allowedCategories.includes(params.kategori)) {
        notFound();
    }

    const projects = await getProjects(params.kategori);
                ...
                <div className="layout-container">
                    <div className="flex overflow-x-auto py-4 gap-4 no-scrollbar">
                        <Link
                            ...
                            className={`px-6 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-colors ${params.kategori === 'tum-projeler' ? 'bg-primary text-white' : 'bg-gray-100 text-text-secondary hover:bg-gray-200'}`}
                        >
                            Tüm Projeler
                        </Link>
                        <Link
                            href="/projeler/tamamlanan-kamu-projeleri"
                            className={`px-6 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-colors ${params.kategori === 'tamamlanan-kamu-projeleri' ? 'bg-primary text-white' : 'bg-green-50 text-green-700 hover:bg-green-100'}`}
                        >
                            Tamamlanan İşler
                        </Link>
                        <Link
                            href="/projeler/devam-eden-altyapi-isleri"
                            className={`px-6 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-colors ${params.kategori === 'devam-eden-altyapi-isleri' ? 'bg-primary text-white' : 'bg-yellow-50 text-yellow-700 hover:bg-yellow-100'}`}
                        >
                            Devam Eden İşler
                        </Link>
                    </div>
