    ...
    return (
        <section className="relative w-full h-[80vh] min-h-[500px] overflow-hidden">
                ...
                <div className="layout-container">
                    <AnimatePresence mode="wait">
                        ...
                        >
                            ...
                            <Link
                                href="/projeler/kategori/tum-projeler"
                                className="inline-flex items-center gap-2 bg-primary hover:bg-primary-dark px-8 py-4 text-white font-semibold rounded-sm"
                            >
                                Projelerimiz <ChevronRight size={20} />
                            </Link>
                        </motion.div>
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-4">
                <button onClick={prevSlide} aria-label="Önceki Slayt" className="p-2 bg-white/20 hover:bg-white/40 text-white rounded-full">
                    <ChevronLeft size={24} />
                </button>
                <div className="flex gap-4">
                    {slides.map((_, idx) => (
                        <button
                            ...
                            className={`w-4 h-4 rounded-full transition-colors flex items-center justify-center ${idx === currentSlide ? "bg-white ring-4 ring-white/30" : "bg-white/40 hover:bg-white/60"}`}
                        />
                    ))}
                </div>
                <button onClick={nextSlide} aria-label="Sonraki Slayt" className="p-2 bg-white/20 hover:bg-white/40 text-white rounded-full">
                        ...
                        <div className="flex gap-2">
                            {slides.map((_, idx) => (
                                <button
                                    ...
                                    onClick={() => setCurrentSlide(idx)}
                                    aria-label={`Slayt ${idx + 1}`}
                                    className={`w-3 h-3 rounded-full ${idx === currentSlide ? "bg-white" : "bg-white/40"
                                        }`}
                                />
                            ))}
                        </div>
                    ...
                    <div className="absolute right-8 top-1/2 -translate-y-1/2 hidden lg:flex flex-col gap-2">
                        {slides.map((slide, idx) => (
                            <button
                                key={idx}
                                onClick={() => setCurrentSlide(idx)}
                                className={`text-right px-4 py-2 text-sm ${idx === currentSlide
